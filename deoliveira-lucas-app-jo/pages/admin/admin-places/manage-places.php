...
<body>
    ...
    <main>
        ...
        try {
            ...
            $query = "SELECT id_lieu, nom_lieu, adresse_lieu, cp_lieu, ville_lieu FROM LIEU ORDER BY id_lieu";
            $statement = $connexion->prepare($query);
            $statement->execute();

            // Vérifier s'il y a des résultats
            if ($statement->rowCount() > 0) {
                echo "<table><tr><th>N°</th><th>Lieu</th><th>Adresse</th><th>Code Postal</th><th>Ville</th><th>Modifier</th><th>Supprimer</th></tr>";

                // Afficher les données dans un tableau
                while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['id_lieu'], ENT_QUOTES, 'UTF-8') . "</td>";
