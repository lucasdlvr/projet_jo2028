...
<body>
    ...
    <main>
        ...
        try {
            ...
            $query = "SELECT nom_epreuve, date_epreuve, heure_epreuve FROM EPREUVE";
            ...
            if ($statement->rowCount() > 0) {
                echo "<table>";
                echo "<tr><th class='color'>Epreuve</th><th>Date</th><th>Heure</th></tr>";

                // Afficher les données dans un tableau
                while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['nom_epreuve'], ENT_QUOTES, 'UTF-8') . "</td>";
                    echo "<td>" . date(format:"d/m/Y", timestamp: strtotime(datetime: $row['date_epreuve'])) . "</td>";
                    echo "<td>" . date(format: "H:i", timestamp: strtotime(datetime: $row['heure_epreuve'])) . "</td>";
                    echo "</tr>";
                }
